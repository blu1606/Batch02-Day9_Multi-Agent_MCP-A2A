...
def retrieve(
    ...
    """
    # Step 1: Chạy semantic + lexical (bọc try-except để chống crash khi thiếu API key)
    try:
        dense_results = semantic_search(query, top_k=top_k * 2)
    except Exception as e:
        print(f"⚠ Semantic search failed: {e}. Chạy tiếp không dùng kết quả dense.")
        dense_results = []

    try:
        sparse_results = lexical_search(query, top_k=top_k * 2)
    except Exception as e:
        print(f"⚠ Lexical search failed: {e}. Chạy tiếp không dùng kết quả sparse.")
        sparse_results = []
    ...
    if use_reranking and merged:
        try:
            ...
            print(f"⚠ Rerank failed: {e}. Sử dụng kết quả merged gốc.")
