...
class TestRagGraph(unittest.TestCase):
    """LangGraph workflow and route checks."""

    def test_route_query_text_domains(self):
        from src.rag_agents import route_query_text

        self.assertEqual(route_query_text("Luật phòng chống ma tuý quy định gì?"), "legal")
        self.assertEqual(route_query_text("Nghệ sĩ nào bị bắt trong vụ việc năm 2024?"), "news")
        self.assertEqual(route_query_text("Tóm tắt tài liệu chính"), "general")

    def test_run_rag_graph_returns_required_keys(self):
        from src import rag_graph

        def fake_retrieve(query, top_k=5, score_threshold=0.3, use_reranking=True):
            return [{
                "content": "Luật quy định trách nhiệm phòng chống ma tuý.",
                "score": 0.9,
                "source": "hybrid",
                "metadata": {"source": "legal.md", "type": "legal"},
            }]

        def fake_generate(query, chunks, context, system_prompt_override=None):
            return "Câu trả lời có căn cứ [1]"

        original_retrieve = rag_graph.retrieve
        original_generate = rag_graph.generate_answer_from_context
        try:
            rag_graph.retrieve = fake_retrieve
            rag_graph.generate_answer_from_context = fake_generate
            result = rag_graph.run_rag_graph("Luật phòng chống ma tuý?", retrieval_params={"top_k": 1})
        finally:
            rag_graph.retrieve = original_retrieve
            rag_graph.generate_answer_from_context = original_generate

        self.assertEqual(result["answer"], "Câu trả lời có căn cứ [1]")
        self.assertEqual(result["retrieval_source"], "hybrid")
        self.assertEqual(result["route"], "legal")
        self.assertEqual(len(result["sources"]), 1)
        self.assertIn("standalone_query", result)
        self.assertIn("errors", result)

    def test_run_rag_graph_falls_back_when_no_sources(self):
        from src import rag_graph
        ...
        try:
            rag_graph.retrieve = fake_retrieve
            rag_graph.generate_answer_from_context = fake_generate
            result = rag_graph.run_rag_graph("Không có nguồn?")
        finally:
            rag_graph.retrieve = original_retrieve
            rag_graph.generate_answer_from_context = original_generate

        self.assertEqual(result["answer"], "Tôi không thể xác minh thông tin này từ nguồn hiện có.")
        self.assertEqual(result["sources"], [])
        self.assertIn("No sources were retrieved.", result["errors"])
